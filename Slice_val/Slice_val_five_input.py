...
# slice_file_path = '../data/inputs/Slice_02.csv'
# lut_file_path = '../data/results/LUT/Slice_02_LUT.csv'
# lut_file_path = 'slice02'
sheet_name = 'slice02'
...
slice_real_data_values = slice_real_data[['Tube Diameter', 'Heated Length', 'Enthalpy value of water', 'Enthalpy value of air' , 'Mass Flux', 'Outlet Quality',]].values

print(slice_real_data_values)
...
# 加载保存的模型权重
pth = torch.load('../model/Transformer_best_model_108.pth',map_location=device)
model.load_state_dict(pth)
model.eval()  # 设置为评估模式

# ————————————————————————————————————————————————————————————得到结果————————————————————————————————————————————————————————————

# normalized_slice_data_2d = data_to_transform.to_numpy(dtype=float)
...
slice_real_file_path
data_lut = pd.read_excel(slice_real_file_path, sheet_name='chf_public_LUT', engine='openpyxl')
# 获取最后一列的数据，排除第一个值
