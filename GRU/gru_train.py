...
feature_columns = ['Tube Diameter', 'Heated Length', 'Enthalpy value of water', 'Enthalpy value of air', 'Mass Flux', 'Inlet Temperature']
label_column = 'CHF'
...
#         self.gru_model = nn.GRU(input_size=d_model, hidden_size=10, num_layers=num_encoder_layers, batch_first=True, bidirectional=True)
...
class GRUModel(nn.Module):
    def ___init__(self, d_model, nhead, num_encoder_layers, dim_feedforward, dropout):
        super(GRUModel, self).__init__()
        self.hidden_size = 10  # 隐层大小
        self.num_layers = num_encoder_layers  # gru层数
...
def train_and_evaluate(model, train_loader, val_loader, optimizer, criterion, epochs):
    ...
    for epoch in range(epochs):
        ...
        if total_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), f'../model/Transformer_best_model_{calIndex}.pth')
...
model = GRUModel(d_model=d_model, nhead=nhead, num_encoder_layers=num_encoder_layers,dim_feedforward=dim_feedforward, dropout=dropout)
...
#
# 保存训练和验证损失
with open(f'../model/Transformer_train_losses_{calIndex}.txt', 'w') as f:
    for loss in train_losses:
        f.write(f"{loss}\n")

with open(f'../model/Transformer_val_losses_{calIndex}.txt', 'w') as f:
...
plt.legend()
plt.grid(True)
plt.savefig('../model/Transformer_loss_plot_{calIndex}.png')
plt.show()
