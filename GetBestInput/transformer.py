...
def train_and_evaluate(model, train_loader, val_loader, optimizer, criterion, epochs, device, model_path):
...
def doTrain(feature_columns = ['Tube Diameter' ,'Heated Length', 'Enthalpy value of water', 'Enthalpy value of air' , 'Enthalpy Temperature','Mass Flux', 'Inlet Subcooling'],epochsCount = 100,model_path = '../model/Transformer_best_model_findBest.pth') :
    # 文件抬头
    calIndex = 'findBest'
    # Load the dataset
    ...
    train_losses, val_losses, best_val_loss = train_and_evaluate(model, train_loader, val_loader, optimizer, criterion, epochs=epochsCount, device= device,model_path = model_path)
    ...
    # plt.legend()
    # plt.grid(True)
    # plt.savefig(f'../model/Transformer_loss_plot_{calIndex}.png')
    # plt.show()


if __name__ == "__main__":
    print("测试transformer")
    doTrain(epochsCount = 1)
